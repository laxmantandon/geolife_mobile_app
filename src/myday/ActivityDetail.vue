...
export default defineComponent({
    ...
    created() {
        ...
        this.get_activity(this.$route.params.id);        
    },
    ...
    methods: {
        ...
        async get_activity(activity: any) {
            var config = {
                method: 'get',
                url: `http://localhost:8000/api/resource/Daily Activity/${activity}?fields=["*"]`,
                headers: {
                    'Authorization': 'token 6c82fbe5cb93440:c43e538a40050ec'
                },
        ...
        async save_activity(activity: any) {
            ...
            await axios(config)
                .then(response => {
                    ...
                    this.showAlert("Alert", `Activity - ${response.data.data.name}`, "Activity Created Successfully.")
                })
                .catch(error => {
