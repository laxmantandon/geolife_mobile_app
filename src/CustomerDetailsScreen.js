      ...
      Geolocation.getCurrentPosition(info => {
        ...
        mylocation = {
          "type": "FeatureCollection", "features": [{
            "type": "Feature", "properties": { "point_type": "circlemarker", "radius": 10 },
  ...
  return (
    <ScrollView style={mstyle.container1}>
      <View style={[mstyle.inputContainer, { paddingVertical: 18, paddingHorizontal: 10, elevation: 8 }]}>
        ...
        <View style={{
          alignItems: 'center', elevation: 4, padding: 10, margin: 5, borderRadius: 5,
          backgroundColor: item.item.count == 0 ? Colors.LIGHT_RED : Colors.LIGHT_GREEN
        }}>
          <Text style={{ color: 'black', fontSize: 15, fontWeight: '700' }}> DGO Visits : {item.item.count == 0 ? 'No' : item.item.count}</Text>
          {item.item.count == 0 ? ('') : (
            <View>
              <Text style={{ color: 'black', fontSize: 13, fontWeight: '500' }}> Last visited by {item.item.geo_mitra_name} on {moment(item.item.posting_date).format('DD MMM-yy')}  </Text>
              {item.item.last_visit ? (
                <Text style={{ color: 'black', textAlign: 'center', fontSize: 13, fontWeight: '500' }}> Last visit for {item.item.last_visit}   </Text>
              ) : ('')}
            </View>
          )}
      ...
      <View style={{ flex: 1, flexDirection: 'row', paddingVertical: 10, marginHorizontal: 10 }}>
        ...
        <Pressable
          style={{
            ...
            flex: 1, borderWidth: .5, borderRadius: 5, paddingHorizontal: 10, paddingVertical: 5, marginRight: 1
